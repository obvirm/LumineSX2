...
impl HttpDownloaderBackend for CurlBackend {
    ...
    fn start_request(&mut self, request: &mut HttpRequest) -> bool {
        ...
        request.start_time = current_ticks() as i64;
...
/// `LOGGER` static — global logging state. Mirrors the file-static
/// variables in `Console.cpp`. We use `LazyLock<Mutex<T>>` so that
/// `.lock().unwrap()` calls auto-deref through to the inner `Mutex`.
static LOGGER: std::sync::LazyLock<std::sync::Mutex<LoggerState>> =
    std::sync::LazyLock::new(|| std::sync::Mutex::new(LoggerState::new()));
...
impl WAVWriter {
    ...
    pub fn close(&mut self) {
        ...
        if let Some(mut f) = self.file.take() {
            ...
            if !write_header_to(&mut f, self.num_channels, self.sample_rate, self.num_frames) {
    ...
    fn write_header(&mut self) -> bool {
        if let Some(f) = self.file.as_mut() {
            write_header_to(f, self.num_channels, self.sample_rate, self.num_frames)
        } else {
            false
        }
    }
...
fn write_header_to(f: &mut File, num_channels: u32, sample_rate: u32, num_frames: u64) -> bool {
    let data_size = (size_of::<i16>() as u64) * (num_channels as u64) * num_frames;
    let header = WavHeader {
        ...
        sample_rate,
        byte_rate: sample_rate * num_channels * size_of::<i16>() as u32,
        block_align: (num_channels * size_of::<i16>() as u32) as u16,
        bits_per_sample: 16,
